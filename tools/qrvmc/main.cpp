...
qrvmc::bytes load_from_hex(const std::string& str)
{
    ...
    return qrvmc::from_hex(str).value();  // Should be validated already.
}
...
int main(int argc, const char** argv) noexcept
{
    ...
    try
    {
        ...
        std::string code_arg;
        int64_t gas = 1000000;
        auto rev = QRVMC_LATEST_STABLE_REVISION;
        std::string input_arg;
        auto create = false;
        auto bench = false;

        CLI::App app{"QRVMC tool"};
        const auto& version_flag = *app.add_flag("--version", "Print version information and exit");
        const auto& vm_option =
            *app.add_option("--vm", vm_config, "QRVMC VM module")->envname("QRVMC_VM");

        auto& run_cmd = *app.add_subcommand("run", "Execute QRVM bytecode")->fallthrough();
        ...
        run_cmd.add_option("--rev", rev, "QRVM revision")
            ->capture_default_str()
            ->check(CLI::Range(static_cast<int>(QRVMC_ZOND), static_cast<int>(QRVMC_MAX_REVISION)));
        ...
        try
        {
            ...
            if (vm_option.count() != 0)
            {
                ...
                vm = VM{qrvmc_load_and_configure(vm_config.c_str(), &ec)};
                if (ec != QRVMC_LOADER_SUCCESS)
                {
                    const auto error = qrvmc_last_error_msg();
                    if (error != nullptr)
                        std::cerr << error << "\n";
                    else
            ...
            if (version_flag)
            {
                ...
                std::cout << "QRVMC " PROJECT_VERSION;
                if (argc >= 1)
                    std::cout << " (" << argv[0] << ")";
                std::cout << "\n";
                return 0;
